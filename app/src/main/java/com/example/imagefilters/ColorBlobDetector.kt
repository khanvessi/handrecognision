...
class ColorBlobDetector {
    ...
    private val mLowerBound: Scalar? = null
    private val mUpperBound: Scalar? = null
    ...
    @JvmName("getSpectrum1")
    fun getSpectrum(): Mat? {
        return spectrum
    }

    @JvmName("getContours1")
    ...
    fun setHsvColor(hsvColor: Scalar) {
        ...
        while (j < maxH - minH) {
            val tmp = byteArrayOf(
                (minH + j).toInt().toByte(), 255.toByte(),
                255.toByte()
            )
